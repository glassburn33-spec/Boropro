...
function generatePlotSVG(scheduleData: StageInputs, scheduleName: string, refLines: ReferenceLines, tempUnit: 'C' | 'F' = 'C'): SVGSVGElement {
  ...
  const titleFontSize = Math.max(36, Math.min(48, width / 20));
  const labelFontSize = Math.max(28, Math.min(36, width / 30));
  const tickFontSize = Math.max(24, Math.min(32, width / 40));
      ...
      <svg width="100%" height="auto" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet" className="border border-stone-600 rounded-lg bg-stone-900 w-full" style={{ minHeight: '300px', maxHeight: '600px', aspectRatio: `${width} / ${height}` }}>
        ...
        {(() => {
          ...
          return filteredMarkers.map((temp) => (
            <g key={`tick-${temp}`}>
              ...
              <text x={margin.left - 15} y={margin.top + scaleY(temp) + 4} textAnchor="end" fill="#fbbf24" fontSize={Math.max(16, (tickFontSize - 1) * 2)}>
                {temp}°{tempUnit}
              </text>
            </g>
          ));
        ...
        {cumulativeTimes.map((time, idx) => (
          <g key={`time-marker-${idx}`}>
            ...
            <text x={margin.left + scaleX(time)} y={margin.top + plotHeight + 20} textAnchor="middle" fill="#fbbf24" fontSize={tickFontSize * 2}>
              {time} min
            </text>
          </g>
        ))}
          ...
          {stageNames.map((name, idx) => {
            ...
            return (
              <g key={`legend-${idx}`}>
                ...
                <text x={legendStartX + 15} y={legendStartY + 9 + idx * legendItemHeight} fill="#fbbf24" fontSize={labelFontSize * 2}>
                  {idx + 1} = {name}
                </text>
              </g>
            );
    ...
    <div className="space-y-6 sm:space-y-8 bg-stone-900/50 p-4 sm:p-8 rounded-lg border border-stone-700">
      ...
      <div className="bg-stone-800/50 border border-stone-600 p-6 rounded-lg">
        ...
        <div className="bg-amber-900/30 border border-amber-700/50 p-3 rounded mb-4">
          <p className="text-sm text-amber-200">Log your schedule before leaving this page</p>
        </div>
